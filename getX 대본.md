...
## 돈(자산)이 오가는 서비스인데 개발 속도가 중요한 MVP 중심 기술을 쓰는 게 맞는가?”

“좋은 질문입니다.  
하지만 CoinRush는 **실제 돈이 오가는 금융 서비스가 아니라 모의 투자 앱**입니다.  
그리고 중요한 점은,  
**자산 검증, 잔고 계산, 주문 체결 같은 금융 핵심 로직은 모두 서버에서 처리된다**는 것입니다.
...
따라서 상태관리 기술은 금융 안정성과는 직접적인 관련이 없습니다.  
오히려 CoinRush처럼 실시간 시세가 빠르게 변하는 앱에서는  
GetX의 **고성능·저지연 업데이트**가  
사용자 경험을 가장 높일 수 있는 선택입니다.

즉, 금융의 안정성은 백엔드가 책임지고,  
Flutter에서는 ‘가장 빠르게 반응하는 UI’를 만드는 것이 핵심이며  
그 목적에 GetX가 가장 적합합니다.”
