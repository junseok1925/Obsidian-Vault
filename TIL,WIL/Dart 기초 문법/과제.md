...
**오류**
- 총 장바구니 금액 (`totalPrice`)이 2만원을 안남을 경우 오류 발생 : 

   `int finalPrice = 0;` 최종 금액이 초기값이 0인데
   `if (totalPrice > 20000)` 2만원 이하 일때 해당 조건문을 거치지 않아 생기는 오류
   
   `int finalPrice = 0;` -> `int finalPrice = totalPrice;`
  -> 최종 금액의 초기값을 총 장바구니 금액 (`totalPrice`) 으로 설정해서 조건문을 거치지 않으면 최종 금액을 총 장바구니 금액으로 지정
