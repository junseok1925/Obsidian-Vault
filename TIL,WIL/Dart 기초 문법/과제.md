...
2. `file.writeAsStringSync(content);` 메서드로 content를 내용으로 파일 생성
...
3. ` studentData.containsKey(inputName)`  : `containsKey()` 메서드로 해당 학생의 이름으로 된 Key 존재 시 조건문에 점수 조회와 동시에 해당 학생의 이름과 점수에 대한 .csv파일 생성
