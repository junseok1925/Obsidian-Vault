...
class Student {
  ...
  static String schoolName = "토평고등학교";
...
void main() {
  ...
  Student student2 = Student("강범석");

  // 인스턴스 변수(name)는 객체마다 다르지만,
  // static 변수(schoolName)는 모든 객체가 동일하게 참조
  print("${student1.name} 학생의 학교는 ${Student.schoolName}임.");
  print("${student2.name} 학생의 학교는 ${Student.schoolName}임.");

  // static 변수 값 변경 → 모든 객체에 영향을 미침
  Student.schoolName = "인창고등학교";

  // student1, student2 모두 같은 새로운 schoolName을 출력
  print("${student1.name} 학생의 새로운 학교는 ${Student.schoolName}임.");
  print("${student2.name} 학생의 새로운 학교는 ${Student.schoolName}임.");
}
