...
- 체크 박스 위젯인 `todo`를 `onTap()`하게 되면 체크박스의 상태를 관리하는`bool` 타입의 변수 `isDone`의 값을 반대로 변경한다 ( `item.isDone = !item.isDone;`: 반대로 변경( false -> true ) )
  (Update)

- `showCupertinoDialog`의 값을 가진 `bool` 타입의 변수 `result` 가 true(삭제버튼을 누른 상태)면 `todoList.removeAt(index);` -> `todoList`의 `index`번째 텍스트 데이터를 삭제 (Delete)