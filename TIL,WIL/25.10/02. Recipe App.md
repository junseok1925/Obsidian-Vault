...
class RecipeMenu extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
      // Extract widget으로 추출한 menuItem 위젯으로 다른 아이콘 텍스트를 가진 동일한 디자인의 메뉴아이탬 생성
...
class menuItem extends StatelessWidget {
  final IconData icon; // 매개변수로 아이콘 데이터 받음
  final String text;   // 매개변수로 텍스트 데이터 받음
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
        // 매개변수로 받은 아이콘 데이터로 아이콘 생성
          Icon(icon, color: Colors.redAccent, size: 30),
          SizedBox(height: 5),
        // 매개변수로 받은 텍스트 데이터로 텍스트 생성
          Text(
            text,
