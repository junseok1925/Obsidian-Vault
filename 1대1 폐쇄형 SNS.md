...
### 2. 왜 사진(`photos`)을 (`rooms`) 안에 하위 컬렉션으로 두었는지

가장 중요한 이유는 **"보안"**과 **"정리"** 때문임

### 1. 데이터의 논리적 소속감 (Hierarchical Relationship)

`photos`는 그 자체로 독립적인 데이터라기보다 **"특정 방에 종속된 추억"**임

- **직관적인 주소:** `/rooms/ABC/photos/123`이라는 경로는 "ABC라는 방의 123번 사진"이라는 의미를 담고 있다
    
- **관리의 용이성:** 나중에 커플이 서비스에서 탈퇴하여 `rooms` 문서를 삭제할 때, 그 하위에 딸린 `photos`들도 한꺼번에 식별하여 정리하기가 훨씬 수월 함
    

### 2. 보안 규칙(Security Rules) 설계의 단순화

Firestore에서 가장 중요한 것은 **"남의 사진을 보지 못하게 하는 것"**입니다.

- **상위 권한 상속:** "이 사진을 볼 권한이 있는가?"를 체크할 때, `photos`가 하위에 있으면 **상위 `rooms` 문서의 `member` 리스트만 확인**하면 된다.
    
- **효율적인 규칙:** 최상위 컬렉션에 있으면 매번 "이 사진이 어느 방 사진이고, 그 방의 멤버는 누구지?"라고 복잡하게 찾아야 하지만, 하위 컬렉션 구조는 경로에 이미 `roomId`가 포함되어 있어 보안 검사가 훨씬 빠르고 저렴해진다 

### 3. 데이터 격리 및 쿼리 최적화 (Data Isolation)

수만 명의 사용자가 사진을 올릴 때, 모든 사진이 한 곳에 섞여 있는 것보다 방별로 나누어져 있는 것이 관리 면에서 유리합니다.

- **방별 타임라인:** 앱에서 "우리 방의 사진첩"을 불러올 때, 전체 사진 중 우리 것을 필터링하는 것보다 우리 방 주소(`rooms/ID/photos`)에 있는 것만 싹 가져오는 것이 성능상 더 안정적
    
- **독립적 확장:** 만약 나중에 특정 커플만 사진이 수천 장이 되어도, 다른 사용자의 사진 쿼리 속도에 전혀 영향을 주지 않는다.
...
## 📂 Firebase Storage 구조 설계도

### 1. 전체 폴더 트리 (Folder Tree)

Plaintext

```
/ (Root)
├── emotion/                 <-- 서비스 공용 데이터 (읽기 전용)
│   ├── happy.png
│   ├── sad.png
│   └── ...
│
└── userSentPhotos/          <-- 사용자 업로드 데이터 (개인 정보)
    └── {roomId}/            <-- 방(커플) 단위 그룹화
        └── {uid}/           <-- 올린 사람 단위 그룹화
            └── {photoId}.jpg <-- 실제 이미지 파일
```
...
|**폴더명**|**관리 주체**|**접근 권한 (보안)**|**역할 및 특징**|
|---|---|---|---|
|**`emotion/`**|개발자(나)|**모든 유저 읽기 가능**|위젯에 쓰일 이모지 사진 보관. 사용자는 읽기만 가능하고 업로드는 불가.|
|**`userSentPhotos/`**|사용자|**해당 방 멤버만 가능**|사용자들이 주고받는 사진의 메인 저장소.|
|**`{roomId}/`**|커플(방)|**방 참여자(A, B)**|보안의 핵심 경계. 내 방 ID 폴더가 아니면 접근 불가.|
|**`{uid}/`**|개별 유저|**작성자 식별**|누가 올렸는지 명확히 구분. 나중에 "내가 올린 사진" 관리 시 유용.|